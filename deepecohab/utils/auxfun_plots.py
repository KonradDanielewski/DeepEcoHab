...
def create_node_trace(
    ...
    for node in animals:
        ...
        ranking_score = (
            ranking_ordinal.loc[node] if ranking_ordinal.loc[node] > 0 else 0.1
        )
...
def prep_network_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    graph_data = (
        df.groupby(level="chased", observed=False)
        .sum(min_count=1)
...
def prep_sum_per_position_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    plot_df = (
        df.melt(ignore_index=False, var_name="animal_id", value_name="y_val")
...
def prep_mean_per_position_df(df: pd.DataFrame) -> pd.DataFrame:
    """Auxfun to prepare visits_per_position data for plotting"""
    plot_df = df.melt(
        ignore_index=False, var_name="animal_id", value_name="y_val"
...
def prep_ranking(ranking_df: pd.DataFrame, phase_range: list[int, int]) -> pd.Series:
    ...
    ranking = ranking_df.loc[
        ("dark_phase", phase_range[-1]), :
    ].iloc[  # TODO: think of a good way to handle it. Pass mode? check which phase was last if all chosen?
        0
    ]
    return ranking
...
def prep_ranking_distribution(ranking_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.index = np.arange(
        -25, 75, 0.1
    )  # Index that should handle most possible rankings
...
def prep_ranking_in_time_df(
    main_df: pd.DataFrame, ranking_in_time: pd.DataFrame, per_hour: bool
) -> pd.DataFrame:
    ...
    if per_hour:
        ...
        multiplier = 0.00333
    index_len = np.ceil(
        (main_df.datetime.iloc[-1] - main_df.datetime.iloc[0]).total_seconds()
        * multiplier
    ).astype(int)
    idx = pd.date_range(main_df.datetime.iloc[0], main_df.datetime.iloc[-1], index_len)
    idx = idx[idx <= ranking_in_time.index[-1]]

    plot_indices = np.searchsorted(
        np.array(ranking_in_time.index), np.array(idx), side="left"
    )
...
def prep_pairwise_arr(
    df: pd.DataFrame, agg_switch: Literal["sum", "mean"]
...
def prep_activity_overtime_sum(df: pd.DataFrame) -> pd.DataFrame:
    ...
    plot_df = (
        df.groupby(["animal_id", "hour"], observed=False)
        .size()
...
def prep_activity_overtime_mean(df: pd.DataFrame) -> pd.DataFrame:
    """Auxfun to prep data for a line plot of activity over hours"""
    plot_df = (
        df.groupby(["day", "hour", "animal_id"], observed=False)
        .size()
        .reset_index(name="count")
    )

    mean_df = (
        plot_df.groupby(["animal_id", "hour"], observed=False).mean().reset_index()
    )
...
def prep_chasing_overtime_mean(chasings_df: pd.DataFrame) -> pd.DataFrame:
    ...
    mean_df = (
        plot_df.groupby(["animal_id", "hour"], observed=False)["count"]
        .mean()
        .reset_index()
    )
    sem_df = (
...
def prep_time_per_cage(cage_occupancy: pd.DataFrame) -> pd.DataFrame:
    ...
    plot_df = pd.concat(
        [
            ...
            cage_occupancy.groupby(by=["hours", "cage", "animal_id"])[
                "time_sum"
            ].mean(),
        ...
        axis=1,
        keys=["time_sum", "time_mean", "lower", "upper"],
    ).reset_index()
