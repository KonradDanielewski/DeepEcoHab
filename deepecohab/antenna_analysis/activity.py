...
def calculate_cage_occupancy(
    ...
    cage_occupancy = None if overwrite else auxfun.load_ecohab_data(cfp, key)
    
    if isinstance(cage_occupancy, pl.LazyFrame):
        return cage_occupancy
    
    binary_lf = auxfun.load_ecohab_data(cfp, 'binary_df')

    cols = ['day', 'hour', 'cage', 'animal_id']

    cage_occupancy = binary_lf.group_by(cols).agg(
        cs.boolean().sum().alias('time_spent')
    ).sort(cols)
...
def calculate_activity(
    ...
    time_per_position_lf = None if overwrite else auxfun.load_ecohab_data(cfp, key)
