...
def plot_cage_position_time(cfp: str, time_per_position: pd.DataFrame, cmap: str = "Set1", save_plot: bool = True):
    """Plot simplebar plot of time spent in each cage.

    Args:
        cfp (str): Path to project config file.
        cmap (str, optional): Color map for bar plot. Defaults to "Set1".
...
def plot_cage_position_visits(cfp: str, visits_per_position: pd.DataFrame, cmap: str = "Set1", save_plot: bool = True):
...
def plot_time_together(cfp: str, time_together: pd.DataFrame, cmap:str = "OrRd", save_plot: bool = True):
    """Plot heatmap of time spent together.

    Args:
        cfp (str): Path to project config file.
        time_together (pd.DataFrame): dataframe generated by deepecohab.calculate_time_together.
        cmap (str, optional): Color map for heatmap. Defaults to "OrRd".
        save_plot (bool, optional): Save plot. Defaults to True.
    """
    # Read config file
    cfg = auxfun.read_config(cfp)
    ...
    for phase_type in plot_data['phase'].unique():
        ...
        _data['dummy_id'] = np.nan # added column to improve the heatmap visualization and readability
        ...
        heatmap_data = (
            ...
            .values
            .reshape(n_phases, n_cages, len(animals_ids), len(animals_ids))
            )

        fig = px.imshow(
            np.floor(heatmap_data), # it does not make sense to plot decimal values imo
            animation_frame=0,
            x=animals_ids,
            y=animals_ids,
            color_continuous_scale=cmap,  
            title=f"Time together in {phase_type_name} phase",
            facet_col=1,
            facet_col_wrap=2
            )
...
def plot_incohort_soc(cfp: str, incohort_soc: pd.DataFrame, cmap: str="OrRd", save_plot: bool = True):
    """Plot heatmap of time spent together.

    Args:
        cfp (str): Path to project config file.
        incohort_soc (pd.DataFrame): dataframe generated by deepecohab.calculate_in_cohort_sociability.
        save_plot (bool, optional): Save plot. Defaults to True.
    """
    # Read config file
    cfg = auxfun.read_config(cfp)
    ...
    for phase_type in plot_data['phase'].unique():
        ...
        fig = fig.update_layout(
                            ...
                            width=800,
                            plot_bgcolor='white',
                            title=dict(text=f"<b>In-cohort sociability during {phase_type_name} phase</b>", x=0.01, y=0.95)
                        )
