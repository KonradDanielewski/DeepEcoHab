...
def create_ecohab_project(
    ...
    antenna_rename_scheme: dict = None,
    ) -> str:
    """Creates the ecohab project directory and config

    Args:
        project_location: path to where the project should be created. Defaults to None.
        experiment_name: name of the experiment. Defaults to "ecohab_project".
        data_path: path to directory that contains raw data (COMxxxxx.txt files). Defaults to None.
    ...
    if os.path.exists(project_location):
        ...
        config_path = Path(project_location) / "config.toml"
        ...
        os.mkdir(Path(project_location) / "plots")
        os.mkdir(Path(project_location) / "data")
    # Create the toml
    config_path = Path(project_location) / "config.toml"
