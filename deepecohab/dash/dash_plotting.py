...
@plot_registry.register(
    "ranking-line", dependencies=["store", "animals", "animal_colors"]
)
...
def network_dominance(cfg: PlotConfig) -> tuple[go.Figure, pl.DataFrame]:
    ...
    connections, nodes = auxfun_plots.prep_network_dominance(
        cfg.store, cfg.animals, cfg.days_range
    )
...
def activity(cfg: PlotConfig) -> tuple[go.Figure, pl.DataFrame]:
    ...
    return plot_factory.plot_activity(
        df, cfg.position_colors, cfg.position_switch, cfg.agg_switch
    )
...
def network_sociability(cfg: PlotConfig) -> tuple[go.Figure, pl.DataFrame]:
    ...
    Returns:
        A tuple containing the Plotly Figure and the processed Polars DataFrame.
    """
    connections = auxfun_plots.prep_network_sociability(
        cfg.store, cfg.animals, cfg.days_range
    )
