...
def plot_mean_line_per_hour(
    ...
    for animal, color in zip(animals, colors):
        ...
        shade_color = color.replace("rgb", "rgba").replace(
            ")", ", 0.2)"
        )  # shaded region is SEM
...
def plot_network_graph(
    ...
    pos = nx.spring_layout(
        G, k=0.1, iterations=200, seed=42, weight=edge_weight, method="energy"
    )
    ...
    for animal in animals:
        match graph_type:
            case "chasings":
                ordinal = (
                    nodes.filter(pl.col("animal_id") == animal).select("ordinal").item()
                )
            case "sociability":
                ordinal = 20
